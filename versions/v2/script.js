...
document
  ...
  .addEventListener("click", tocarSomClick);
...
function atualizarRanking() {
  ...
  rankingLocal.sort((a, b) => b.pontuacao - a.pontuacao);

  for (let i = 0; i < rankingLocal.length; i++) {
    listaRanking.innerHTML += `
      <div id="nomeRanking">
        <span>${i + 1}º</span>
        <span>${rankingLocal[i].nome}</span>
        <span>${rankingLocal[i].pontuacao}</span>
      </div>
    `;
  }
}
